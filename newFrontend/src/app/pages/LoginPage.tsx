...
export function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setErrorMsg("");
    try {
      const { token } = await authApi.login(email.trim(), password.trim());
      localStorage.setItem('token', token);
      navigate(from);
    } catch (error: any) {
      console.error("Login failed:", error);
      setErrorMsg(error?.response?.data?.error || "Login failed due to invalid credentials.");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="bg-white rounded-lg shadow-lg p-12">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <p className="text-center text-sm text-gray-600">
              ...
              <Link to="/register" state={{ from }} className="text-blue-900 hover:underline">
                Register here
              </Link>
            </p>
          </form>
